local utils = require("mp.utils")

require("mp.options")

local options = {}
options.UseSystemTrash = true
options.MoveToFolder = false
options.ShowConfirmation = true

if package.config:sub(1, 1) == "/" then
	options.DeletedFilesPath = utils.join_path(os.getenv("HOME"), "delete_file")
	ops = "unix"
else
	options.DeletedFilesPath = utils.join_path(os.getenv("USERPROFILE"), "delete_file")
	ops = "win"
end

read_options(options)

del_list = {}

function move_to_trash(file_path)
	local cmd = {}

	if ops == "win" then
		cmd = {
			"powershell",
			"-Command",
			string.format(
				"Add-Type -AssemblyName Microsoft.VisualBasic; "
					.. "[Microsoft.VisualBasic.FileIO.FileSystem]::DeleteFile('%s', "
					.. "'OnlyErrorDialogs', 'SendToRecycleBin')",
				file_path:gsub("/", "\\")
			),
		}
	else
		local trash_commands = {
			{ "trash", file_path }, -- trash-cli (Linux)
			{ "gio", "trash", file_path }, -- GNOME
			{ "kioclient5", "move", file_path, "trash:/" }, -- KDE5
			{ "kioclient", "move", file_path, "trash:/" }, -- KDE4
			{
				"osascript",
				"-e",
				string.format('tell application "Finder" to delete POSIX file "%s"', file_path),
			},
		}

		for _, trash_cmd in ipairs(trash_commands) do
			local test = utils.subprocess({ args = { "which", trash_cmd[1] }, capture_stdout = true })
			if test.status == 0 then
				cmd = trash_cmd
				break
			end
		end

		-- Fallback: create ~/.Trash folder (macOS style)
		if #cmd == 0 then
			local trash_dir = utils.join_path(os.getenv("HOME"), ".Trash")
			if not utils.file_info(trash_dir) then
				os.execute(string.format('mkdir -p "%s"', trash_dir))
			end
			local filename = file_path:match("([^/]+)$")
			local trash_path = utils.join_path(trash_dir, filename)

			-- Handle duplicate names
			local counter = 1
			while utils.file_info(trash_path) and counter <= 100 do
				local name, ext = filename:match("^(.+)(%.[^%.]+)$")
				if ext then
					trash_path = utils.join_path(trash_dir, string.format("%s_%d%s", name, counter, ext))
				else
					trash_path = utils.join_path(trash_dir, string.format("%s_%d", filename, counter))
				end
				counter = counter + 1
			end

			return os.rename(file_path, trash_path)
		end
	end

	if #cmd > 0 then
		local result = utils.subprocess({ args = cmd })
		return result.status == 0
	end

	return false
end

function createDirectory()
	if not utils.file_info(options.DeletedFilesPath) then
		if not os.execute(string.format('mkdir "%s"', options.DeletedFilesPath)) then
			print("failed to create folder")
		end
	end
end

function contains_item(l, i)
	for k, v in pairs(l) do
		if v == i then
			if options.ShowConfirmation then
				mp.osd_message("undeleting current file", 2)
			end
			l[k] = nil
			return true
		end
	end
	if options.ShowConfirmation then
		mp.osd_message("deleting current file", 2)
	end
	return false
end

function mark_delete()
	local work_dir = mp.get_property_native("working-directory")
	local file_path = mp.get_property_native("path")
	local s = file_path:find(work_dir, 0, true)
	local final_path
	if s and s == 0 then
		final_path = file_path
	else
		final_path = utils.join_path(work_dir, file_path)
	end
	if not contains_item(del_list, final_path) then
		table.insert(del_list, final_path)
	end
end

function delete()
	if #del_list == 0 then
		return
	end

	if options.UseSystemTrash then
		for i, v in pairs(del_list) do
			print("Moving to trash: " .. v)
			local success = move_to_trash(v)
			if success then
				mp.osd_message("Moved to trash: " .. v:match("([^/\\]+)$"), 1)
			else
				mp.osd_message("Failed to trash: " .. v:match("([^/\\]+)$"), 2)
				print("Failed to move to trash: " .. v)
			end
		end
	elseif options.MoveToFolder then
		createDirectory()
		for i, v in pairs(del_list) do
			print("moving: " .. v)
			local _, filename = utils.split_path(v)

			-- Handle duplicate filenames
			for i = 1, 100 do
				if i > 1 then
					if filename:find("[.].+$") then
						filename = filename:gsub("([.].+)$", string.format("_%d%%1", i))
					else
						filename = string.format("%s_%d", filename, i)
					end
				end
				local movedPath = utils.join_path(options.DeletedFilesPath, filename)
				local fileInfo = utils.file_info(movedPath)
				if not fileInfo then
					os.rename(v, movedPath)
					break
				end
			end
		end
	else
		-- Permanent deletion
		for i, v in pairs(del_list) do
			print("PERMANENTLY deleting: " .. v)
			mp.osd_message("PERMANENTLY deleting: " .. v:match("([^/\\]+)$"), 2)
			os.remove(v)
		end
	end
end

function showList()
	local delString = "Delete Marks:\n"
	for _, v in pairs(del_list) do
		local dFile = v:gsub("/", "\\")
		delString = delString .. dFile:match("\\*([^\\]*)$") .. "; "
	end
	if delString:find(";") then
		mp.osd_message(delString)
		return delString
	elseif showListTimer then
		showListTimer:kill()
	end
end

showListTimer = mp.add_periodic_timer(1, showList)
showListTimer:kill()

function list_marks()
	if showListTimer:is_enabled() then
		showListTimer:kill()
		mp.osd_message("", 0)
	else
		local delString = showList()
		if delString and delString:find(";") then
			showListTimer:resume()
			print(delString)
		else
			showListTimer:kill()
		end
	end
end

mp.add_key_binding("DEL", "delete_file", mark_delete)
mp.add_key_binding("alt+DEL", "list_marks", list_marks)
mp.add_key_binding("ctrl+shift+DEL", "clear_list", function()
	mp.osd_message("Undelete all", 2)
	del_list = {}
end)

mp.register_event("shutdown", delete)
